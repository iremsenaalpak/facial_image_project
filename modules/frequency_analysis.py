...
from modules.evaluation import ensure_same_size
...
def compare_frequency_analysis(original_image, processed_image):
    ...
    original_image, processed_image = ensure_same_size(
        original_image,
        processed_image
    )
...
def frequency_table_to_html(table):
    ...
    for item in table:
        rows += f"""
        <tr>
            <td style="border:1px solid #ccc; padding:8px;">{item["metric"]}</td>
            <td style="border:1px solid #ccc; padding:8px;">{item["original"]}</td>
            <td style="border:1px solid #ccc; padding:8px;">{item["processed"]}</td>
            <td style="border:1px solid #ccc; padding:8px;">{item["difference"]}</td>
        </tr>
        """
